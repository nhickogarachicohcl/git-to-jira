...
function parseFilesFromDiff(diffOutput) {
  const files = [];
...
function getCommitDetails(commit) {
  try {
    ...
    console.error(`Error getting diff for ${commit.sha}:`, error.message);
...
function formatForLLM(jiraKey, branchRef, flaggedCommits) {
  const branchName = branchRef.replace('refs/heads/', '');
