...
export async function addComment(issueID: string, comment: string): Promise<void> {
    try {
...
export async function changeStatus(issueID: string, statusId: string): Promise<void> {
    try {
        await axios.post(`/issue/${issueID}/transitions`, { transition: { id: statusId } });
    } catch (error) {
